...
<form method="POST" action="{{ url('traspaso') }}" autocomplete="off" id ="formulario">
@csrf
<div class ="divpagina">
    <div class ="titulobotonagregar">
        <div><h5 class="titulos"><i class="fas fa-angle-double-right"></i> TRASPASO</h5></div>
    </div>
    ...
    <div class = "tamano_letra_fatura">
        ...
        <div class=" form-group col-sm-2 labelliquidaciones">
        ...
        <div class="form-group col-md-4 labelliquidaciones">
            <label for="name"  class="titulonrofac">{{ __('Sucursal Origen:') }}</label>
            <select name="sucursal_origen"  type = "number" class="form-control input_facturacion " required>
                <option value=""></option>
                @foreach($sucu as $su)
                <option  value="{{$su->nro_sucursal}}" >{{$su->descripcion}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group col-md-4 labelliquidaciones">
            <label for="name"  class="titulonrofac">{{ __('Sucursal Destino:') }}</label>
            <select name="sucursal_destino"  type = "number" class="form-control input_facturacion " required>
                <option value=""></option>
                @foreach($sucu as $su)
                <option  value="{{$su->nro_sucursal}}" >{{$su->descripcion}}</option>
                @endforeach
